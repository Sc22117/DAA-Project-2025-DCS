...
def upload_university_data(institute, file_path):
...
def detect_clashes(institute, max_iterations=3):
    ...
    while iteration < max_iterations:
        ...
        if clash_found:
            ...
            resolve_venue_clashes(institute)
...
def generate_timetable_pipeline(institute, start_time_str, end_time_str, period_duration, num_breaks, break_duration, num_days, file_path, output_path):
    data_uploaded = upload_university_data(institute, file_path)

    if not data_uploaded:
        print("⏩ Skipping generation. Proceeding to final report.")
        generate_excel_reports(institute, output_file=output_path)
        return
    ...
    upload_venue_timetable_to_firebase(
        institute, start_time_str, end_time_str,
        period_duration, num_breaks, break_duration, num_days
    )

    # Assign lunch break only once
    ...
    generate_excel_reports(institute, output_file=output_path)
    print("✅ Timetable pipeline completed.")
